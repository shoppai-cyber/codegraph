...
export class DatabaseConnection {
  ...
  private static openConfigured(
    ...
    try {
      ...
      // Fork: skipped on read-only opens, same contract as healBulkNodeLoad
      // above — a read-only consumer never writes, and only a writable
      // session can have left an oversized WAL worth healing.
      if (!options.readOnly) {
