...
| Python | Grove node DSL | exact source → node-group identity → helper/zone flow | R + X | 🔬 **source-structural resolver candidate** — top-level `@node_tree(id=..., target=...)` groups and nested Repeat/Simulation/For-Each Zones become file-qualified `component` nodes; same-file group calls retain one `calls` edge per source site; `contains` records exact zone ownership; `decorates` preserves the existing Python function source. Synthetic targets are resolved only within the source file, so duplicate historical snapshots cannot steal an edge. SHA-pinned 7,313-line large probe: **35 groups + 4 zones + 46 framework-resolved calls**; an exact-path query rendered `roof` → `solve` → `skel_collapse` at call sites 7258 and 6884 with zero substitutions from the older same-hash snapshot. Computed/stacked/duplicate decorators, dynamic calls, cross-file group joins, tuple/attribute flow, Grove compilation, and Blender evaluation remain deliberately silent. Small/medium rendered probes and the prescribed consumer A/B remain required before ✅. |
